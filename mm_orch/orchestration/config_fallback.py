            ...
            try:
                ...
                if self._validate_router_config(config):
                    ...
                    return ConfigFallbackResult(
                        config=config,
                        used_fallback=False,
                        config_source="phase_b"
                    )
                else:
                    logger.warning("Phase B router config invalid, falling back", path=str(phase_b_path))
        ...
        if phase_b_path.exists():
            try:
                ...
                if self._validate_workflow_registry_config(config):
                    logger.info("Loaded Phase B workflow registry configuration", path=str(phase_b_path))
                    return ConfigFallbackResult(
                        config=config,
                        used_fallback=False,
                        config_source="phase_b"
        ...
        if phase_b_path.exists():
            try:
                ...
                if self._validate_model_registry_config(config):
                    logger.info("Loaded Phase B model registry configuration", path=str(phase_b_path))
                    return ConfigFallbackResult(
                        config=config,
                        used_fallback=False,
                        config_source="phase_b"
        ...
        if phase_b_path.exists():
            try:
                ...
                if self._validate_tool_registry_config(config):
                    logger.info("Loaded Phase B tool registry configuration", path=str(phase_b_path))
                    return ConfigFallbackResult(
                        config=config,
                        used_fallback=False,
                        config_source="phase_b"
