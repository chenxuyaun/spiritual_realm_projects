...
class PhaseBOrchestrator:
    ...
    def _initialize_phase_b_components(self) -> PhaseBComponents:
        ...
        if components.step_registry:
            try:
                ...
                self._register_workflows_from_config(
                    components.workflow_registry,
                    workflow_config
                )
                logger.info(
        ...
        if version == "v3":
            try:
                ...
                if all([vectorizer_path, classifier_path, costs_path]):
                    if all(Path(p).exists() for p in [vectorizer_path, classifier_path, costs_path]):
                        return RouterV3(
                            vectorizer_path,
                            classifier_path,
                            costs_path,
    ...
    def _route_request(self, request: UserRequest, state: State) -> Tuple[str, float]:
        ...
        if self.phase_b.router:
            try:
                workflow, confidence, candidates = self.phase_b.router.route(
                    request.query,
                    state
                )
...
def get_phase_b_orchestrator(
    ...
    if _phase_b_orchestrator is None:
        _phase_b_orchestrator = PhaseBOrchestrator(
            phase_b_components,
            legacy_runtime,
            config_dir
        )
