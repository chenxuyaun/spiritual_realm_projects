            ...
            try:
                ...
                if hasattr(runtime, "model_manager") and runtime.model_manager:
                    summary = runtime.model_manager.infer(
                        self.model_name, f"summarize: {content}", max_new_tokens=150
                    )
                else:
    ...
    def execute(self, state: State, runtime: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if fetch_result.success and fetch_result.content:
                ...
                return {"docs": docs, "citations": citations}
            else:
    ...
    def execute(self, state: State, runtime: Any) -> Dict[str, Any]:
        ...
        if not citation_refs:
            ...
            for ref in citation_refs:
                ...
                if ref_num < 1 or ref_num > max_citation_num:
                    errors.append(
                        f"Invalid citation reference [{ref}] (valid range: 1-{max_citation_num})"
                    )
        ...
        if key_sentences:
            ...
            if citation_ratio < 0.3:  # At least 30% of key sentences should have citations
                errors.append(
                    f"Insufficient citations: only {sentences_with_citations}/{len(key_sentences)} key points have citations"
                )
        ...
        return {"validation_passed": validation_passed, "validation_errors": errors}
        ...
        try:
            ...
            if hasattr(runtime, "model_manager") and runtime.model_manager:
                answer = runtime.model_manager.infer(
                    self.model_name, prompt, max_new_tokens=300, temperature=0.7
                )
...
class SummarizeToAnswerStep(BaseStep):
    ...
    def execute(self, state: State, runtime: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(runtime, "model_manager") and runtime.model_manager:
                summary = runtime.model_manager.infer(
                    self.model_name, f"summarize: {content}", max_new_tokens=200
                )
            else:
                # Fallback: simple truncation
                summary = self._simple_summarize(content)
            ...
            return {"final_answer": summary, "citations": citations}
...
class LessonExplainStep(BaseStep):
    ...
    def __init__(
        self, model_name: str = "gpt2", max_new_tokens: int = 1500, temperature: float = 0.7
    ):
        """
    ...
    def execute(self, state: State, runtime: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if structured_lesson:
                ...
                return {"lesson_explain_structured": structured_lesson.to_json()}
            else:
                ...
                return {"teaching_text": response, "meta": updated_meta}
            ...
            return {"teaching_text": fallback_text, "meta": updated_meta}

    def _build_json_prompt(
        self, topic: str, objectives: List[str], outline: List[str], difficulty: str, language: str
    ) -> str:
        """
...
Please output JSON directly without additional explanatory text:"""
    ...
    def _generate_template_json(self, topic: str, difficulty: str, language: str) -> str:
        """
        ...
        if language == "zh":
            ...
            sections = [
                ...
                LessonSection(
                    ...
                    questions=[
                        f"Please explain the meaning of {topic}",
                        f"Please give examples of {topic} applications",
                    ],
