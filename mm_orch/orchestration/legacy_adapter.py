...
class LegacyWorkflowAdapter(BaseStep):
    ...
    def _inject_runtime_dependencies(self, runtime: Any) -> None:
        ...
        if hasattr(self.workflow, 'conversation_manager') and hasattr(runtime, 'conversation_manager'):
    ...
    def _convert_result_to_state(self, result: WorkflowResult, original_state: State) -> Dict[str, Any]:
        """
        Convert WorkflowResult to State updates.
        ...
        if result.result is not None:
            ...
            is_search_workflow = (
                "search" in workflow_type or
                "searchqa" in workflow_type.replace("_", "").replace("-", "") or
                hasattr(self.workflow, 'workflow_type') and 
                self.workflow.workflow_type and
                "search" in str(self.workflow.workflow_type.value).lower()
            )
            ...
            if is_search_workflow:
                ...
                if "sources" in result.metadata:
                    updates["citations"] = [
                        source.get("url", "") for source in result.metadata["sources"]
                        if source.get("url")
                    ]
