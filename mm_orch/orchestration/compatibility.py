...
def execute_legacy_workflow(
    ...
    state: State,
    runtime: Optional[LegacyRuntime] = None
) -> State:
    """
...
def convert_workflow_result_to_state(
    ...
    original_state: Optional[State] = None
) -> State:
    """
    ...
    if result.metadata:
        ...
        if "sources" in result.metadata:
            state["citations"] = [
                source.get("url", source.get("doc_id", ""))
...
class LegacyWorkflowExecutor:
    ...
    def execute_with_result(
        ...
        logger.info(
            f"Legacy workflow '{workflow.name}' completed (Phase A mode)",
