...
class KVCacheManager:
    ...
    def get_cache(
        ...
        if cache_key in self._cache:
            ...
            logger.debug(
                f"Cache miss for conversation={conversation_id}, model={model_name}"
            )
    ...
    def evict_lru(self, required_memory: float):
        ...
        while (
            self._cache and 
            self._memory_used_mb + required_memory > self.config.max_memory_mb and
            ...
            logger.debug(
                f"Evicted cache for {cache_key}, freed={cache_entry.memory_mb:.2f}MB"
            )
