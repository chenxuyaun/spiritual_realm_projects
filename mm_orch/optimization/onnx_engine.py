...
class ONNXEngine:
    ...
    def _validate_conversion(
        ...
        try:
            ...
            if not np.allclose(pytorch_output_np, onnx_output_np, rtol=rtol, atol=atol):
                ...
                raise RuntimeError(
                    f"ONNX model outputs don't match PyTorch outputs "
    ...
    def load_model(self, onnx_path: str, model_name: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to load ONNX model {model_name}: {e}",
                exc_info=True
            )
            ...
            logger.error(
                f"Unexpected error loading ONNX model {model_name}: {e}",
                exc_info=True
            )
            # Clean up partial initialization
            self._session = None
    ...
    def infer(
        ...
        try:
            ...
            output_dict = {
                name: output
                for name, output in zip(self._output_names, onnx_outputs)
            }
            
            logger.info(
                f"ONNX Runtime inference completed in {inference_time:.2f}s"
            )
            ...
            logger.error(
                f"ONNX Runtime inference failed: {e}",
                exc_info=True
            )
    ...
    def _get_available_providers(self) -> List[str]:
        ...
        try:
            ...
            filtered_providers = [
                provider for provider in self.config.execution_providers
