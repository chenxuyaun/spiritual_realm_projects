...
class GPUManager:
    ...
    def detect_gpus(self, force_refresh: bool = False) -> List[GPUInfo]:
        ...
        for device_id in range(gpu_count):
            try:
                ...
                logger.error(
                    f"Failed to get info for GPU {device_id}: {e}",
                    exc_info=True
                )
    ...
    def allocate_gpus(
        ...
        if min_memory_mb:
            healthy_gpus = [
                gpu for gpu in healthy_gpus 
                if gpu.available_memory_mb >= min_memory_mb
            ]
            
            if not healthy_gpus:
                raise RuntimeError(
                    f"No GPUs with at least {min_memory_mb}MB available memory"
                )
        ...
        logger.info(
            f"Allocated {len(allocated_gpu_ids)} GPUs {allocated_gpu_ids} "
            f"for {strategy}"
        )
    ...
    def get_all_gpu_metrics(self) -> Dict[int, Dict[str, float]]:
        ...
        for gpu in gpus:
            try:
                ...
                logger.error(
                    f"Failed to get metrics for GPU {gpu.device_id}: {e}",
                    exc_info=True
                )
    ...
    def recover_from_failure(
        ...
        healthy_gpus = [
            gpu for gpu in gpus 
            if gpu.is_healthy and gpu.device_id not in self._failed_gpus
        ]
        ...
        if len(healthy_gpus) >= total_gpus_needed:
            try:
                ...
                logger.info(
                    f"Recovery successful: reallocated to GPUs {new_allocation}"
                )
