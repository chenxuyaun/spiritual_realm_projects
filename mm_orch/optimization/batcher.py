            ...
            if "input_ids" in inputs:
                ...
                if isinstance(input_ids, list):
                    ...
                    original_lengths.append(
                        len(input_ids) if hasattr(input_ids, "__len__") else 0
                    )
...
class DynamicBatcher:
    ...
    def stop(self, timeout: float = 5.0):
        ...
        with self._lock:
            for model_name, req_queue in self._request_queues.items():
                if req_queue:
                    logger.info(
                        f"Processing {len(req_queue)} remaining requests for {model_name}"
                    )
    ...
    def _process_individual_request(self, request: InferenceRequest):
        ...
        logger.debug(
            f"Processed individual request {request.request_id} "
    ...
    def _adapt_batch_size(self):
        ...
        if avg_latency > target_max:
            ...
            self._current_batch_size = max(
                ...
                self._current_batch_size - reduction
            )
            reason = "high_latency"
