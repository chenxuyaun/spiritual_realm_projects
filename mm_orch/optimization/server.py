    ...
    def start(self):
        ...
        try:
            ...
            if self.server_config.preload_models:
                logger.info(f"Pre-loading {len(self.server_config.preload_models)} models...")
                self._preload_models()

            # Start worker thread
            logger.info("Starting worker thread...")
            self._worker_thread = threading.Thread(
                target=self._process_requests, name="InferenceServerWorker", daemon=False
            )
    ...
    def stop(self, timeout: Optional[int] = None):
        ...
        if self._worker_thread and self._worker_thread.is_alive():
            ...
            if self._worker_thread.is_alive():
                logger.warning(
                    f"Worker thread did not complete within {timeout}s, " "forcing shutdown"
                )
    ...
    def _preload_models(self):
        ...
        for model_name in self.server_config.preload_models:
            try:
                ...
                logger.info(
                    f"Model {model_name} pre-loaded successfully " f"(engine: {result.engine_used})"
                )

            except Exception as e:
                logger.error(f"Failed to pre-load model {model_name}: {e}", exc_info=True)
                self._degradation_reasons.append(f"Failed to pre-load model {model_name}: {e}")
            ...
            with self._result_lock:
                if request_id in self._result_futures:
                    ...
                    if future["status"] == "success":
                        ...
                        raise RuntimeError(f"Request {request_id} failed: {future['error']}")
