...
class VLLMEngine:
    ...
    def load_model(
        ...
        try:
            ...
            if tp_size > 1:
                try:
                    gpu_ids, strategy = self._gpu_manager.allocate_gpus(tensor_parallel=tp_size)
                    self._allocated_gpus = gpu_ids
                    logger.info(f"Allocated {len(gpu_ids)} GPUs for tensor parallelism: {gpu_ids}")
            ...
            logger.info(f"Model {model_name} loaded successfully with vLLM " f"in {load_time:.2f}s")
            ...
            logger.error(f"Failed to load model {model_name} with vLLM: {e}", exc_info=True)
    ...
    def generate(
        self, prompts: List[str], sampling_params: Optional[Any] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.debug(
                f"Generating with vLLM for {len(prompts)} prompts " f"(model: {self._loaded_model})"
            )
            ...
            for vllm_output in vllm_outputs:
                ...
                output = {
                    ...
                    "finish_reason": (
                        vllm_output.outputs[0].finish_reason if vllm_output.outputs else None
                    ),
                    "tokens_generated": (
                        len(vllm_output.outputs[0].token_ids) if vllm_output.outputs else 0
                    ),
                }
