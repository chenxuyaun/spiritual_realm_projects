...
class ConfigurationManager:
    ...
    def _apply_non_critical_updates(
        ...
        if old_min != new_min or old_max != new_max:
            try:
                ...
                if new_min > new_max:
                    raise ValueError(
                        f"min_batch_size ({new_min}) must be <= max_batch_size ({new_max})"
                    )
    ...
    def _validate_parameter_value(self, parameter: str, value: Any):
        ...
        # Create temporary config object to validate (triggers __post_init__ validation)
        if section == "batcher":
            _ = replace(self._config.batcher, **{param: value})
        elif section == "cache":
            _ = replace(self._config.cache, **{param: value})
        elif section == "tuner":
            _ = replace(self._config.tuner, **{param: value})
        elif section == "server":
            _ = replace(self._config.server, **{param: value})
        else:
            raise ValueError(f"Unknown configuration section: {section}")
