...
class AutoTuner:
    ...
    def _init_metrics(self):
        ...
        try:
            ...
            self.current_batch_size_metric = Gauge(
                'auto_tuning_batch_size',
                'Current batch size set by auto-tuner'
            )
            ...
            self.current_cache_size_metric = Gauge(
                'auto_tuning_cache_size_mb',
                'Current cache size in MB set by auto-tuner'
            )
    ...
    def analyze_performance(
        ...
        if metrics is None:
            return TuningRecommendations(
                rationale="Insufficient performance data for analysis"
            )
    ...
    def apply_tuning(
        ...
        if recommendations.batch_timeout_ms and self.batch_timeout_callback:
            ...
            # Expose in metrics
            self._record_tuning_metric("batch_timeout_ms", old_value, recommendations.batch_timeout_ms)
    ...
    def check_for_degradation(self) -> bool:
        ...
        current_metrics = self.performance_monitor.get_operation_metrics(
            "inference",
            window_seconds=60  # Check recent performance
        )
        ...
        latency_degradation = (
            current_metrics.p95_latency_ms > self._baseline_latency * 1.2
        )
        throughput_degradation = (
            current_metrics.throughput_rps < self._baseline_throughput * 0.8
        )
    ...
    def _tuning_loop(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if (recommendations.batch_size or 
                    recommendations.batch_timeout_ms or 
                    recommendations.cache_size_mb):
