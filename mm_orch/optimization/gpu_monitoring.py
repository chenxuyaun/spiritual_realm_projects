...
class GPUMonitor:
    ...
    def __init__(
        ...
        logger.info(
            f"GPUMonitor initialized with interval={monitoring_interval}s"
        )
    ...
    def _monitoring_loop(self):
        ...
        while not self._stop_monitoring.is_set():
            try:
                ...
                logger.error(
                    f"Error in GPU monitoring loop: {e}",
                    exc_info=True
                )
    ...
    def _collect_and_record_metrics(self):
        ...
        try:
            ...
            unhealthy_count = sum(
                1 for is_healthy, _ in health_status.values() if not is_healthy
            )
            
            if unhealthy_count > 0:
                logger.warning(
                    f"GPU health check: {unhealthy_count}/{gpu_count} GPUs unhealthy"
                )
            else:
            ...
            logger.error(
                f"Failed to collect and record GPU metrics: {e}",
                exc_info=True
            )
