...
class DeepSpeedEngine:
    ...
    def load_model(
        ...
        try:
            ...
            if tp_size > 1 or pp_size > 1:
                try:
                    gpu_ids, strategy = self._gpu_manager.allocate_gpus(
                        tensor_parallel=tp_size, pipeline_parallel=pp_size
                    )
                    ...
                    if pp_size > 1:
                        ...
                        logger.info(f"Pipeline parallelism allocation: {self._pipeline_allocation}")

                    logger.info(f"Allocated {len(gpu_ids)} GPUs for {strategy}: {gpu_ids}")
            ...
            logger.info(
                f"Model {model_name} loaded successfully with DeepSpeed " f"in {load_time:.2f}s"
            )
            ...
            logger.error(f"Failed to load model {model_name} with DeepSpeed: {e}", exc_info=True)
    ...
    def infer(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            logger.info(f"DeepSpeed inference completed in {inference_time:.2f}s")

            return output_dict

        except Exception as e:
            logger.error(f"DeepSpeed inference failed: {e}", exc_info=True)
    ...
    def generate(
        ...
        **kwargs,
        ...
        """
        if self._model is None or self._loaded_model is None:
            raise RuntimeError("No model loaded. Call load_model() before generate().")
        ...
        try:
            ...
            for i, (prompt, gen_ids) in enumerate(zip(prompts, generated_ids)):
                ...
                generated_text = self._tokenizer.decode(gen_ids, skip_special_tokens=True)
            ...
            logger.error(f"DeepSpeed generation failed: {e}", exc_info=True)
