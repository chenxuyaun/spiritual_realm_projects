...
class PerformanceMonitor:
    ...
    def __init__(self, max_history_seconds: int = 3600, resource_sample_interval: int = 10):
        """
    ...
    def record_latency(
        self, operation: str, latency_ms: float, metadata: Optional[Dict[str, Any]] = None
    ):
        """
    ...
    def get_percentiles(
        self, operation: str, window_seconds: Optional[int] = None
    ) -> Dict[str, float]:
        """
        ...
        with self._latency_lock:
            ...
            if window_seconds:
                ...
                latencies = [r.latency_ms for r in records if r.timestamp >= cutoff]
            else:
    ...
    def get_resource_trends(self, window_seconds: int = 300) -> ResourceTrends:
        """
        ...
        with self._resource_lock:
            cutoff = datetime.now() - timedelta(seconds=window_seconds)
            recent_snapshots = [s for s in self._resource_snapshots if s.timestamp >= cutoff]
            ...
            if recent_snapshots[0].gpu_memory_mb:
                ...
                for gpu_id in gpu_ids:
                    ...
                    if gpu_util_values:
                        trends.avg_gpu_utilization[gpu_id] = sum(gpu_util_values) / len(
                            gpu_util_values
                        )
    ...
    def get_operation_metrics(
        ...
        with self._latency_lock:
            ...
            time_span = (
                filtered_records[-1].timestamp - filtered_records[0].timestamp
            ).total_seconds()
