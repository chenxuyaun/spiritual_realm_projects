...
class OTelTracer:
    ...
    def trace_request(self, request_id: str, **attributes):
        ...
        with self._tracer.start_as_current_span(
            "request", attributes={"request.id": request_id, **attributes}
        ) as span:
            # Record start time
    ...
    def trace_workflow(self, workflow_name: str, **attributes):
        ...
        with self._tracer.start_as_current_span(
            f"workflow.{workflow_name}", attributes={"workflow.name": workflow_name, **attributes}
        ) as span:
            try:
                yield span
                span.set_status(Status(StatusCode.OK))
    ...
    @contextmanager
    def trace_inference(
        self, model_name: str, engine: str, batch_size: Optional[int] = None, **attributes
    ):
        """
        ...
        span_attributes = {"inference.model": model_name, "inference.engine": engine, **attributes}

        if batch_size is not None:
            span_attributes["inference.batch_size"] = batch_size

        with self._tracer.start_as_current_span(
            f"inference.{model_name}", attributes=span_attributes
        ) as span:
            try:
                yield span
                span.set_status(Status(StatusCode.OK))
    ...
    def record_event(
        self, span: Optional[Span], event_name: str, attributes: Optional[Dict[str, Any]] = None
    ):
        """
    ...
    def set_span_attribute(self, span: Optional[Span], key: str, value: Any):
        """
