...
def _apply_env_overrides(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "MUAI_MON_PROMETHEUS_ENABLED" in os.environ:
        prometheus_config["enabled"] = os.environ["MUAI_MON_PROMETHEUS_ENABLED"].lower() in (
            "true",
            "1",
            "yes",
        )
    ...
    if "MUAI_MON_TRACING_ENABLED" in os.environ:
        tracing_config["enabled"] = os.environ["MUAI_MON_TRACING_ENABLED"].lower() in (
            "true",
            "1",
            "yes",
        )
    ...
    if "MUAI_MON_ANOMALY_ENABLED" in os.environ:
        anomaly_config["enabled"] = os.environ["MUAI_MON_ANOMALY_ENABLED"].lower() in (
            "true",
            "1",
            "yes",
        )
    if "MUAI_MON_ANOMALY_LATENCY_THRESHOLD" in os.environ:
        anomaly_config["latency_threshold_ms"] = float(
            os.environ["MUAI_MON_ANOMALY_LATENCY_THRESHOLD"]
        )
    ...
    if "MUAI_MON_ANOMALY_MEMORY_THRESHOLD" in os.environ:
        anomaly_config["memory_threshold_percent"] = float(
            os.environ["MUAI_MON_ANOMALY_MEMORY_THRESHOLD"]
        )
    ...
    if "MUAI_MON_SERVER_ENABLED" in os.environ:
        server_config["enabled"] = os.environ["MUAI_MON_SERVER_ENABLED"].lower() in (
            "true",
            "1",
            "yes",
        )
