...
class WorkflowOrchestrator:
    ...
    def _register_default_workflows(self) -> None:
        """注册默认工作流"""
        # SearchQA工作流
        self.register_workflow(SearchQAWorkflow(
            model_manager=self.model_manager
        ))
        
        # LessonPack工作流
        self.register_workflow(LessonPackWorkflow(
            model_manager=self.model_manager
        ))
        
        # ChatGenerate工作流
        self.register_workflow(ChatGenerateWorkflow(
            model_manager=self.model_manager
        ))
        
        # RAGQA工作流
        self.register_workflow(RAGQAWorkflow(
            model_manager=self.model_manager
        ))
        
        # SelfAskSearchQA工作流
        self.register_workflow(SelfAskSearchQAWorkflow(
            model_manager=self.model_manager
        ))
        
        logger.debug(
            "Registered default workflows",
            workflows=[wf.value for wf in self._workflows.keys()]
        )
    ...
    def execute_workflow(
        ...
        ctx = ExecutionContext(
            request=UserRequest(query=parameters.get("query", "")),
        ...
        try:
            ...
            if not workflow:
                raise OrchestrationError(
                    f"Workflow '{workflow_type.value}' is not registered"
                )
    ...
    def _get_strategy_suggestion(
        ...
        try:
            ...
            logger.warning(
                "Failed to get strategy suggestion",
                error=str(e)
            )
            return None
