...
class ThroughputBenchmark:
    ...
    def measure_concurrent_throughput(
        ...
        """
        logger.info(
            f"Measuring concurrent throughput for {model_name} "
            f"(concurrency={num_concurrent})"
        )
    ...
    def measure_batch_throughput(
        ...
        """
        logger.info(
            f"Measuring batch throughput for {model_name} "
    ...
    def run_throughput_suite(
        ...
        for level in concurrent_levels:
            concurrent_result = self.measure_concurrent_throughput(
                generate_fn, prompts, model_name, get_output_tokens,
        ...
        for size in batch_sizes:
            batch_result = self.measure_batch_throughput(
                batch_generate_fn, prompts, model_name, get_output_tokens,
                batch_size=size
            )
