...
class LatencyBenchmark:
    ...
    def measure_e2e_latency(self, generate_fn: Callable[[str], Any], prompt: str) -> float:
        """
        测量端到端延迟
    ...
    def run_latency_suite(
        ...
        for i in range(self.test_runs):
            ...
            # Tokens/s
            tps = self.measure_tokens_per_second(generate_fn, prompt, get_output_tokens)
            ...
            logger.debug(
                f"Run {i+1}: TTFT={ttft*1000:.2f}ms, " f"TPS={tps:.2f}, E2E={e2e*1000:.2f}ms"
            )
