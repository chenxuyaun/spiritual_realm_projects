...
class StructuredLesson:
    ...
    def from_json(cls, data: Dict[str, Any]) -> "StructuredLesson":
        ...
        return cls(topic=data["topic"], grade=data["grade"], sections=sections)
    ...
    def validate(self) -> Tuple[bool, List[str]]:
        ...
        if len(self.sections) < 3:
            errors.append(f"Lesson must have at least 3 sections, found {len(self.sections)}")

        # Check content requirement
        has_content = any(section.has_content() for section in self.sections)
        if not has_content:
            errors.append("Lesson must contain at least one section with examples or questions")
