    ...
    def _rule_based_decompose(self, query: str, max_questions: int) -> List[SubQuestion]:
        ...
        # Pattern 2: Compare/contrast questions
            ...
            if match:
                ...
                sub_questions.append(SubQuestion(
                    question=f"What is {entity1.strip()}?",
                    order=0
                ))
                sub_questions.append(SubQuestion(
                    question=f"What is {entity2.strip()}?",
                    order=1
                ))
                sub_questions.append(SubQuestion(
        ...
        # Pattern 3: Pros and cons
            ...
            # Extract the topic
            match = re.search(r'(?:pros and cons|advantages and disadvantages)\s+of\s+(.+?)[\?\.]', query, re.IGNORECASE)
            ...
            sub_questions.append(SubQuestion(
                question=f"What are the disadvantages of {topic}?",
                order=1
            ))
        
        # Pattern 4: Multiple question words
        elif re.search(r'\b(how|why|what|when|where|who)\b.*\b(how|why|what|when|where|who)\b', query_lower):
...
Comprehensive answer:"""
    ...
    def _rule_based_synthesize(
        ...
        # Add introduction
        parts.append(f"To answer your question about '{original_query[:100]}...', here's what I found:\n")
    ...
    def _generate_summary(self, answers: List[Dict[str, str]]) -> str:
        ...
        if len(answers) == 1:
            return answers[0]['answer'][:200] + "..." if len(answers[0]['answer']) > 200 else answers[0]['answer']
...
class SelfAskSearchQAWorkflow(BaseWorkflow):
    ...
    def __init__(
        ...
        self.search_qa_workflow = search_qa_workflow or SearchQAWorkflow(
            ...
            max_search_results=max_search_results_per_question
        )
    ...
    def execute(self, parameters: Dict[str, Any]) -> WorkflowResult:
        ...
        try:
            ...
            logger.error(
                "SelfAskSearchQA workflow failed",
                error=str(e),
                query=query[:50]
            )
            return self._create_result(
                ...
                status="failed",
                error=str(e),
                include_sub_answers=include_sub_answers
            )
    ...
    def _step_decompose(self, ctx: SelfAskContext) -> SelfAskContext:
        ...
        try:
            ...
            sub_questions = self.decomposer.decompose(
                ctx.original_query,
                max_questions=ctx.max_sub_questions
            )
    ...
    def _step_search_sub_questions(
        ...
        step = SelfAskStep(
            name="search",
            success=False,
            input_count=len(ctx.sub_questions)
        )
        ...
        try:
            ...
            for i, sub_q in enumerate(ctx.sub_questions):
                logger.info(f"  Searching sub-question {i+1}/{len(ctx.sub_questions)}: {sub_q.question[:40]}...")
    ...
    def _step_synthesize(self, ctx: SelfAskContext) -> SelfAskContext:
        ...
        step = SelfAskStep(
            name="synthesize",
            success=False,
            input_count=len(successful_answers)
        )
    ...
    def _fallback_synthesis(self, ctx: SelfAskContext) -> str:
        ...
        """
        successful = [(sq, ans) for sq, ans in zip(ctx.sub_questions, ctx.sub_answers) if sq.success and ans]
