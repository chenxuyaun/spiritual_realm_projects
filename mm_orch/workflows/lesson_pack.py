        ...
        try:
            ...
            if not ctx.explanation:
                return self._create_result(
                    ctx,
                    status="partial",
                    error="Failed to generate explanation content"
                )
            ...
            logger.error(
                "LessonPack workflow failed",
                error=str(e),
                topic=topic[:50]
            )
            return self._create_result(
                ctx,
                status="partial" if ctx.plan else "failed",
                error=str(e)
            )
...
Teaching Plan:"""
        ...
        if self.model_manager:
            try:
                ...
                return self._clean_generated_text(plan, "教学计划:" if language == "zh" else "Teaching Plan:")
            except Exception as e:
    ...
    def _generate_explanation(
        ...
        if include_examples:
            if language == "zh":
                ...
                example_instruction = "Please include specific examples where appropriate to aid understanding."
...
Explanation:"""
        ...
        if self.model_manager:
            try:
                ...
                return self._clean_generated_text(explanation, "讲解内容:" if language == "zh" else "Explanation:")
            except Exception as e:
