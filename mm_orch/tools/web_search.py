...
class WebSearchTool:
    ...
    def search(
        self, query: str, max_results: int = 5, region: str = "wt-wt", safesearch: str = "moderate"
    ) -> List[SearchResult]:
        """
        ...
        logger.info("Performing web search", query=query[:100], max_results=max_results)
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                raise NetworkError(f"Search library not available: {e}", context={"query": query})
            except Exception as e:
                last_error = e
                logger.warning(
                    f"Search attempt {attempt + 1} failed", error=str(e), query=query[:50]
                )
    ...
    def search_news(
        ...
        try:
            ...
            raw_results = list(ddgs.news(query, max_results=max_results, region=region))
