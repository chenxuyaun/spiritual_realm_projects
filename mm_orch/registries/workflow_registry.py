...
class WorkflowRegistry:
    ...
    def get(self, name: str) -> WorkflowDefinition:
        ...
        if name not in self._workflows:
            ...
            raise KeyError(
                f"Workflow '{name}' not found in registry. " f"Available workflows: {available}"
            )
    ...
    def get_by_tag(self, tag: str) -> List[WorkflowDefinition]:
        ...
        """
        return [definition for definition in self._workflows.values() if tag in definition.tags]
    ...
    def _validate_workflow(self, definition: WorkflowDefinition) -> None:
        ...
        if "start" not in graph:
            raise ValueError(f"Workflow '{definition.name}' graph must have a 'start' node")
...
def get_workflow_registry(step_registry: Optional[Dict[str, Any]] = None) -> WorkflowRegistry:
    ...
    if _global_workflow_registry is None:
        if step_registry is None:
            raise ValueError("step_registry must be provided when creating workflow registry")
