...
class RouterV3:
    ...
    def __init__(
        self, vectorizer_path: str, classifier_path: str, costs_path: str, lambda_cost: float = 0.1
    ):
        """
    ...
    def _load_costs(self) -> Dict[str, float]:
        ...
        if not self.costs_path.exists():
            logger.warning(f"Cost statistics not found: {self.costs_path}. " "Using default costs.")
            return {}
        ...
        try:
            ...
            logger.debug(f"Loaded cost statistics for {len(costs)} workflows")
            return costs

        except Exception as e:
            logger.error("Failed to load cost statistics", path=str(self.costs_path), error=str(e))
            return {}
