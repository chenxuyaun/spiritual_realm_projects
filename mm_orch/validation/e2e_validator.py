...
class E2EValidator:
    ...
    def validate_search_qa(self, test_case: SearchQATestCase) -> ValidationResult:
        """
        验证SearchQA场景
        ...
        try:
            ...
            if test_case.expected_keywords:
                ...
                found_keywords = sum(
                    1 for kw in test_case.expected_keywords if kw.lower() in response_lower
                )
    ...
    def validate_lesson_pack(self, test_case: LessonPackTestCase) -> ValidationResult:
        """
        验证LessonPack场景
    ...
    def validate_multi_turn(self, test_case: ConversationTestCase) -> ValidationResult:
        """
        验证多轮对话场景
        ...
        try:
            ...
            if test_case.context_keywords and responses:
                ...
                found_context = sum(
                    1 for kw in test_case.context_keywords if kw.lower() in last_response_lower
                )
            ...
            # 检查所有响应非空
            checks["all_responses_not_empty"] = all(len(r.strip()) > 0 for r in responses)
    ...
    def load_test_cases(filepath: str) -> Dict[str, List[TestCase]]:
        ...
        if "search_qa" in data:
            result["search_qa"] = [SearchQATestCase(**case) for case in data["search_qa"]]

        if "lesson_pack" in data:
            result["lesson_pack"] = [LessonPackTestCase(**case) for case in data["lesson_pack"]]

        if "conversation" in data:
            result["conversation"] = [ConversationTestCase(**case) for case in data["conversation"]]

        return result

    @staticmethod
    def save_results(results: List[ValidationResult], filepath: str) -> None:
        """
        保存验证结果到JSON文件
