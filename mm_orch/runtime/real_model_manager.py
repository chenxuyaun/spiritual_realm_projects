...
class RealModelManager:
    ...
    def __init__(
        ...
        self._memory_monitor = MemoryMonitor(
            gpu_warning_threshold=memory_threshold, gpu_critical_threshold=0.95
        )
    ...
    def load_model(self, model_config: ModelConfig, force_reload: bool = False) -> LoadedModel:
        """
        加载模型
        ...
        with self._lock:
            ...
            try:
                ...
                self._model_cache[model_key] = CachedModel(
                    loaded_model=loaded_model, last_accessed=datetime.now()
                )
                self._stats["loads"] += 1
    ...
    def _check_memory_before_load(self, model_config: ModelConfig) -> None:
        ...
        if snapshot.gpu_percent > self.memory_threshold * 100:
            logger.warning(
                f"GPU memory usage high ({snapshot.gpu_percent:.1f}%), " "attempting cleanup"
            )
    ...
    def switch_device(self, model_name: str, target_device: str) -> bool:
        """
        切换模型设备
        ...
        with self._lock:
            ...
            try:
                ...
                logger.info(f"Model {model_name} moved from {current_device} to {target_device}")
                return True
