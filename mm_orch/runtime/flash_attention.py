...
class FlashAttentionManager:
    ...
    def _check_availability(cls) -> None:
        ...
        if not torch.cuda.is_available():
            cls._info = FlashAttentionInfo(available=False, reason_unavailable="CUDA not available")
        ...
        try:
            ...
            cls._info = FlashAttentionInfo(
                available=False, reason_unavailable=f"Failed to check GPU properties: {e}"
            )
            cls._available = False
            return
        ...
        try:
            ...
            cls._info = FlashAttentionInfo(
                available=True, version=version, cuda_version=torch.version.cuda
            )
            cls._available = True
            ...
            cls._info = FlashAttentionInfo(
                ...
                cuda_version=torch.version.cuda,
                reason_unavailable=(
                    "flash-attn package not installed. " "Install with: pip install flash-attn"
                ),
            )
            cls._available = False
    ...
    @classmethod
    def get_attention_implementation(cls, prefer_flash: bool = True) -> str:
        """
        获取推荐的注意力实现
    ...
    def configure_model_for_flash_attention(
        cls, model_kwargs: Dict[str, Any], enable_flash: bool = True
    ) -> Dict[str, Any]:
        """
