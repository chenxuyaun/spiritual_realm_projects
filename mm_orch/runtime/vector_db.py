...
class VectorDBManager:
    ...
    def __init__(
        self, dimension: int = 384, index_type: str = "flat", storage_path: Optional[str] = None
    ):
        """
        初始化向量库管理器
    ...
    def create_index(
        self, dimension: Optional[int] = None, index_type: Optional[str] = None
    ) -> None:
        """
        创建向量索引
        ...
        if not self._faiss_available:
            raise ResourceError(
                "FAISS is not available", context={"install_hint": "pip install faiss-cpu"}
            )
        ...
        self._logger.info(
            f"Creating vector index", context={"dimension": dim, "index_type": idx_type}
        )
        ...
        # 创建元数据
        self._metadata = IndexMetadata(dimension=dim, index_type=idx_type)
        ...
        self._logger.info(
            f"Vector index created successfully", context={"dimension": dim, "index_type": idx_type}
        )
    ...
    def _get_embedding(self, document: Document) -> np.ndarray:
        ...
        raise ValidationError("Document has no embedding", context={"doc_id": document.doc_id})
            ...
            try:
                ...
                if embedding.shape[0] != self.dimension:
                    self._logger.warning(
                        ...
                        context={"expected": self.dimension, "got": embedding.shape[0]},
                    )
                    continue
                ...
                self._logger.warning(
                    f"Skipping document without embedding: {doc.doc_id}", context={"error": str(e)}
                )
                continue
    ...
    def add_document(self, document: Document, embedding: Optional[np.ndarray] = None) -> bool:
        """
        添加单个文档到向量库
    ...
    def search(self, query_vector: np.ndarray, top_k: int = 5) -> List[Tuple[Document, float]]:
        ...
        if self._index is None:
            raise ResourceError(
                "Vector index not created", context={"hint": "Call create_index() first"}
            )
        ...
        if query_vector.shape[0] != self.dimension:
            raise ValidationError(
                ...
                context={"expected": self.dimension, "got": query_vector.shape[0]},
            )
        ...
        self._logger.debug(
            f"Search completed", context={"top_k": top_k, "results_count": len(results)}
        )

        return results

    def search_by_text(
        self, query_embedding: np.ndarray, top_k: int = 5, threshold: Optional[float] = None
    ) -> List[Document]:
        """
        根据查询嵌入检索文档（简化接口）
    ...
    def save_index(self, path: Optional[str] = None) -> None:
        ...
        if self._index is None:
            raise ResourceError(
                "No index to save", context={"hint": "Create and populate index first"}
            )
        ...
        if save_path is None:
            raise StorageError(
                "No storage path specified", context={"hint": "Provide path or set storage_path"}
            )
        ...
        try:
            ...
            for doc_id, doc in self._documents.items():
                doc_dict = {"content": doc.content, "metadata": doc.metadata, "doc_id": doc.doc_id}
                # 不保存嵌入向量（太大），需要时重新生成
            ...
            mapping_data = {"id_to_idx": self._id_to_idx, "idx_to_id": self._idx_to_id}
            with open(mapping_file, "w", encoding="utf-8") as f:
                json.dump(mapping_data, f, indent=2)

            self._logger.info(
                f"Index saved successfully",
                context={"path": str(save_path), "document_count": len(self._documents)},
            )

        except Exception as e:
            raise StorageError(f"Failed to save index: {e}", context={"path": str(save_path)})
    ...
    def load_index(self, path: Optional[str] = None) -> None:
        ...
        if not load_path.exists():
            raise StorageError(
                f"Storage path does not exist: {load_path}", context={"path": str(load_path)}
            )
        ...
        try:
            ...
            if not index_file.exists():
                raise StorageError(
                    f"Index file not found: {index_file}", context={"path": str(load_path)}
                )
            ...
            raise StorageError(f"Failed to load index: {e}", context={"path": str(load_path)})
    ...
    def rebuild_index(self) -> None:
        ...
        self._logger.info(f"Index rebuilt", context={"document_count": len(self._documents)})
...
def get_vector_db(
    dimension: int = 384, index_type: str = "flat", storage_path: Optional[str] = None
) -> VectorDBManager:
    """
    获取全局向量库管理器实例
    ...
    if _global_vector_db is None:
        _global_vector_db = VectorDBManager(
            dimension=dimension, index_type=index_type, storage_path=storage_path
        )
