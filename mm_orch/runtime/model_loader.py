...
class ModelLoader:
            ...
            if torch.cuda.is_available():
                ...
                if ":" in preferred:
                    ...
                    if device_id < torch.cuda.device_count():
                        ...
                        logger.warning(
                            f"GPU {device_id} not available, using cuda:0"
                        )
                        return "cuda:0"
                return preferred
            else:
    ...
    def load_model(
        ...
        logger.info(
            f"Loading model {config.model_name} on {device} with dtype {dtype}"
        )
    ...
    def _load_model_internal(
        ...
        logger.info(
            f"Model loaded successfully. Memory footprint: "
            f"{memory_footprint / 1024**3:.2f} GB"
        )
