...
class SymbolGroundingModule:
    ...
    def _query_episodic_memory(self, symbol: str) -> List[GroundingCandidate]:
        ...
        try:
            ...
            if matching_episodes:
                ...
                avg_importance = sum(ep.importance for ep in matching_episodes) / len(matching_episodes)
    ...
    def _normalize_probabilities(self, candidates: List[GroundingCandidate]) -> List[GroundingCandidate]:
        """
    ...
    def _calculate_initial_confidence(self, candidates: List[GroundingCandidate]) -> float:
        ...
        grounded_candidates = [
            ...
            if c.concept_id is not None or len(c.episode_ids) > 0
        ]
    ...
    def update_grounding(
        ...
        if is_consistent:
            ...
            grounding.confidence = min(
                1.0,
                grounding.confidence + self._config.confidence_growth_rate
            )
            ...
            grounding.confidence = max(
                0.0,
                grounding.confidence - self._config.confidence_decay_rate
            )
    ...
    def _find_best_matching_candidate(
        ...
        for candidate in candidates:
            similarity = self._calculate_context_similarity(
                context, candidate.perceptual_features
            )
    ...
    def resolve_ambiguity(
        ...
        best_candidate, _ = self._find_best_matching_candidate(
            grounding.groundings, context
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        grounded_count = sum(1 for g in self._groundings.values() 
        ...
        if total_symbols > 0:
            ...
            avg_candidates = sum(len(g.groundings) for g in self._groundings.values()) / total_symbols
    ...
    def merge_groundings(self, symbol1: str, symbol2: str) -> Optional[SymbolGrounding]:
        ...
        for candidate in grounding2.groundings:
            ...
            for existing in grounding1.groundings:
                if (existing.concept_id == candidate.concept_id and 
                    existing.concept_id is not None):
                    # Merge episode IDs
