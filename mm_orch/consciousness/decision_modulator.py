...
class DecisionModulator:
    ...
    def __init__(
        ...
        self._config = DecisionModulatorConfig.from_dict(config) if config else DecisionModulatorConfig()
    ...
    def get_modifiers(self) -> DecisionModifiers:
        ...
        exploration_bias = max(-0.3, min(0.3, 
            -state.pleasure * 0.15 + (state.arousal - 0.5) * 0.2
        ))
        
        # Calculate confidence threshold adjustment
        # High pleasure and dominance = lower threshold (more confident)
        confidence_threshold = max(-0.2, min(0.2,
    ...
    def load_state(self, state: Dict[str, Any]) -> None:
        ...
        if "decision_log" in state:
            self._decision_log = [
                DecisionLog.from_dict(log) for log in state["decision_log"]
            ]
