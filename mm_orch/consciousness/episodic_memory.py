...
class EpisodicMemory:
    ...
    def retrieve_by_context_similarity(
        ...
        if not query_context:
            ...
            sorted_episodes = sorted(
                self._episodes.values(), key=lambda e: e.timestamp, reverse=True
            )
        ...
        for key in common_keys:
            ...
            # Type match gets partial score
                ...
                if isinstance(query_value, str) and isinstance(episode_value, str):
                    if (
                        query_value.lower() in episode_value.lower()
                        or episode_value.lower() in query_value.lower()
                    ):
                        total_score += 0.5
                    else:
                        total_score += 0.2
    ...
    def _calculate_pad_distance(
        ...
        distance = math.sqrt((p1 - p2) ** 2 + (a1 - a2) ** 2 + (d1 - d2) ** 2)
    ...
    def _prune(self) -> int:
        ...
        def episode_value(ep: Episode) -> float:
            ...
            return ep.importance * 0.5 + access_factor * 0.3 + recency_factor * 0.2

        # Sort episodes by value (ascending) - lowest value first
        sorted_episodes = sorted(
            self._episodes.values(), key=lambda e: (episode_value(e), e.timestamp)
        )
    ...
    def _extract_context_patterns(self, episodes: List[Episode]) -> List[Dict[str, Any]]:
        ...
        for key, count in sorted(key_counts.items(), key=lambda x: -x[1]):
            ...
            if frequency >= 0.2:
                ...
                common_values = sorted(value_counts.items(), key=lambda x: -x[1])[
                    :5
                ]  # Top 5 values
                ...
                patterns.append(
                    {
                        ...
                        "common_values": [{"value": v, "count": c} for v, c in common_values],
                    }
                )
    ...
    def get_frequently_accessed(
        ...
        frequent_episodes = [
            ep for ep in self._episodes.values() if ep.access_count >= min_access_count
        ]

        # Sort by access count (highest first), then by timestamp (most recent first)
        frequent_episodes.sort(key=lambda e: (-e.access_count, -e.timestamp))
    ...
    def get_recently_accessed(
        ...
        recent_episodes.sort(key=lambda e: -e.last_accessed if e.last_accessed is not None else 0)
