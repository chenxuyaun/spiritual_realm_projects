...
class Metacognition:
    ...
    def __init__(self):
        ...
        self._strategy_records: Dict[str, StrategyRecord] = {
            name: StrategyRecord(strategy_name=name)
    ...
    def update_progress(self, task_id: str, progress: float, checkpoint: Optional[str] = None) -> bool:
        """
    ...
    def complete_task(self, task_id: str, success: bool, metrics: Optional[Dict[str, float]] = None) -> Optional[TaskMonitor]:
        """
    ...
    def get_strategy_suggestion(self, task: Task) -> StrategySuggestion:
        ...
        for name, config in self.STRATEGIES.items():
            if task_type in config["applicable_to"] or self._is_type_compatible(task_type, config["applicable_to"]):
    ...
    def _calculate_strategy_score(self, strategy_name: str, complexity: float, config: Dict) -> float:
    ...
    def record_strategy_result(self, strategy_name: str, success: bool, score: float, execution_time: float) -> None:
        """
    ...
    def get_strategy_stats(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            name: {
                ...
                "average_time": record.total_time / record.usage_count if record.usage_count > 0 else 0.0,
            }
