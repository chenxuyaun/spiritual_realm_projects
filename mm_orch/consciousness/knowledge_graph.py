...
class KnowledgeGraph:
    ...
    def add_relationship(self, relationship: Relationship) -> str:
        ...
        if relationship.relationship_id in self._relationships:
            raise ValueError(f"Relationship with ID '{relationship.relationship_id}' already exists")
        ...
        if len(self._relationships) >= self._config.max_relationships:
            raise ValueError(f"Maximum number of relationships ({self._config.max_relationships}) reached")
    ...
    def find_path(
        ...
        while queue:
            ...
            for rel_id in self._outgoing_index.get(current_id, []):
                ...
                if relationship_types is not None and rel.relationship_type not in relationship_types:
                    continue
    ...
    def _infer_transitive(
        ...
        direct_rels = self.get_relationships(node_id, direction="outgoing", relationship_type="is_a")
        
        for rel1 in direct_rels:
            # Get "is_a" relationships from the target
            indirect_rels = self.get_relationships(rel1.target_id, direction="outgoing", relationship_type="is_a")
    ...
    def _infer_siblings(
        ...
        parent_rels = self.get_relationships(node_id, direction="outgoing", relationship_type="is_a")
        
        for parent_rel in parent_rels:
            parent_id = parent_rel.target_id
            
            # Get other children of this parent
            sibling_rels = self.get_relationships(parent_id, direction="incoming", relationship_type="is_a")
            ...
            for sibling_rel in sibling_rels:
                ...
                if (node_id, sibling_id, "related_to") not in existing_pairs and \
                   (sibling_id, node_id, "related_to") not in existing_pairs:
    ...
    def _infer_inheritance(
        ...
        for parent_rel in parent_rels:
            ...
            property_rels = self.get_relationships(parent_id, direction="outgoing", relationship_type="has_property")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        avg_rels = (total_rels * 2) / node_count if node_count > 0 else 0.0  # *2 because each rel connects 2 nodes
    ...
    def validate_integrity(self) -> Dict[str, Any]:
        ...
        for rel_id, rel in self._relationships.items():
            if rel.source_id not in self._nodes:
                issues.append(f"Relationship {rel_id} references non-existent source node {rel.source_id}")
            if rel.target_id not in self._nodes:
                issues.append(f"Relationship {rel_id} references non-existent target node {rel.target_id}")
        ...
        for node_id, rel_ids in self._outgoing_index.items():
            for rel_id in rel_ids:
                if rel_id not in self._relationships:
                    issues.append(f"Outgoing index for {node_id} references non-existent relationship {rel_id}")
        
        for node_id, rel_ids in self._incoming_index.items():
            for rel_id in rel_ids:
                if rel_id not in self._relationships:
                    issues.append(f"Incoming index for {node_id} references non-existent relationship {rel_id}")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "KnowledgeGraph":
        ...
        graph._total_relationships_created = data.get("total_relationships_created", len(graph._relationships))
